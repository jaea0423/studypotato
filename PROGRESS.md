...
> 마지막 업데이트: 2026-06-25 (PIN 간편로그인 인증 모델 설계 확정 / 언어 설정에 일본어 추가)
> 다음 세션 시작 시 이 파일을 먼저 읽어주세요.

## 🎯 다음에 시작할 작업 (TOP PRIORITY)

### 🔐 새 인증 모델: 비밀번호(필수) + PIN으로 간편로그인(기기별, 선택) — ✅ 설계 확정 (2026-06-25)

**배경**:
- 기존: 6자리 PIN = 비밀번호 → 학번 노출 시 100만 조합으로 약함
- 변경: **비밀번호(강력)** = 본인 인증 기준선, **PIN(6자리)** = "PIN으로 간편로그인" (등록 기기에서만 쓰는 빠른 로그인)
...
**✅ 확정된 결정사항 (2026-06-25, 사용자 합의 완료)**

1. **PIN은 "계정 단위"가 아니라 "기기 단위"** — PIN 값을 `registered_devices`에 기기별로 저장. 기기마다 다른 PIN 가능. (한 기기 PIN 유출돼도 다른 기기 안전)
2. **PIN 등록 권유는 "첫 로그인 시"** 표시 (가입 직후도 사실상 첫 로그인이라 포함). "이 기기에서 PIN으로 간편로그인 할래요?" 권유.
3. **기본값 = PIN 해제** (보안 우선). 권유를 "나중에/안 함" 하면 다시 안 띄움 → 설정에서 직접 켜기.
4. **비밀번호 정책**: 최소 8자 + 영문 + 숫자 + 특수문자 (대소문자 구분 없음)
5. **PIN 5회 실패 = 그 기기 등록 해제** (registered_devices row 삭제). 잠금 타이머(pin_locked_until) 불필요. 이후 비밀번호로 다시 로그인 → 재등록.
6. **약한 PIN(000000, 123456 등) 차단 안 함** — 본인이 허락한 기기 + 5회 제한 한정이라 수용. (유일 위험: 기기 도난 시 5회 추측 — 인지 완료)
7. **PIN 변경·해제 시 = 현재 PIN 확인 / PIN 분실 시 = 비밀번호로 로그인 후 재설정**
8. **세션 유지: 탭 닫으면 로그아웃** — Supabase 세션을 localStorage가 아닌 sessionStorage에 저장. (부작용: 새 탭은 로그인 안 된 상태 — 모바일 우선이라 수용)
9. **기기 관리 화면 제공** — 등록 기기 목록 + 현재 기기 표시 + **사용자가 기기 이름(라벨) 변경 가능** + 개별 해제 + 다른 기기 모두 해제.
10. **공용 PC 경고** — PIN 등록 권유 옆에 "본인 기기에서만 사용하세요" 안내.
11. **명칭**: 기능명 "PIN으로 간편로그인", 비밀값 "PIN(6자리 숫자)".

**📌 다음 보안 업그레이드 후보 (v2, 지금 안 함)**: 패스키(passkey / WebAuthn) = 지문·얼굴 인증. 훔칠 비밀번호 없고 피싱(phishing)에 강함. 1인 개발 부담이라 출시 후 검토.
...
| 케이스 | 입력 |
|---|---|
| 신규 가입 | 메일 OTP → 비밀번호(필수) 설정 → (첫 로그인이므로) "이 기기 PIN 간편로그인" 권유 |
| 등록 기기 + PIN 설정함 | PIN 6자리 |
| 등록 기기 + PIN 미설정 | 비밀번호 |
| 새 기기 | 메일 OTP + 비밀번호 → 기기 등록 → "이 기기 PIN 간편로그인" 권유 |
...
- [ ] **로그인 화면**:
  ...
  - PIN 화면: 6셀 + 하단 "비밀번호로 로그인" 링크 (PIN 잊음 처리)
  - 비밀번호 화면: 일반 input
  - PIN 검증은 반드시 서버(`verify_device_pin`)에서. localStorage는 분기 힌트일 뿐
- [ ] **세션 저장소를 sessionStorage로** (탭 닫으면 로그아웃) — supabase client storage 옵션 변경

#### 3단계. 마이페이지 (프로필 메뉴) — "PIN으로 간편로그인" 설정
- [ ] 신규 화면 또는 설정 토글 (PIN 설정/변경/해제, 이 기기 기준)
  - 미설정: "이 기기 PIN 등록" → 6셀 두 번 입력 → `set_device_pin`
  - 설정됨: 현재 PIN 확인 → 새 PIN 두 번 입력 (`set_device_pin`)
  - "PIN 해제" → 현재 PIN 확인 후 `clear_device_pin`
...
- [x] OTP 8자리 → 6자리 (Supabase Dashboard 설정) — ✅ 해결됨 (2026-06-25, 6자리 정상 수신)
- [ ] consents.consent_type CHECK 제약 재정의 (campus, rules 추가) — task #76
- [x] 메일 지연 원인 진단 — ✅ 해결됨 (2026-06-25, 정상화. 재발 시 재진단)
...
- ✅ 감자사이 멘토링 시스템 화면 구현 (커밍순 더미)
  ...
  - **mentoring-detail.html**: 보라 그라데이션 헤더 + 모집 진행률 + 멘토 정보 + 조건 자동 체크 + 추첨 안내 + 지원 모달 (한 줄 메시지 50자)
  - dashboard 홈 탭에 감자사이 진입 배너 (보라 그라데이션 + COMING SOON)
  - preview/index.html에 3개 카드 추가
  - 모든 액션은 "🚧 커밍순 — 정식 출시 후" 토스트
  - 인증 가드 + visibility:hidden 표준 패턴 적용
  - 보라톤 디자인 (mentor-purple #7C4DFF) — 매칭/팀 시스템과 시각적 구분
- ✅ 7가지 피드백 반영
...
### 메일 지연 (1분 30초) — ✅ 해결됨 (2026-06-25, 정상화)
원인 후보 (확률 순):
...
### ✅ OTP 인증코드 6자리 정상 (해결됨 2026-06-25) — 아래는 당시 기록
...
### 🤝 감자사이 — 멘토링 카드 + 랜덤 추첨 (정식 출시 +3개월)

**이름 확정**: 감자사이 (감자들 사이의 관계)

**디자인 모델 (사용자 확정)**: 멘토링 카드 + 조건 필터 + 랜덤 추첨
- 선착순도 멘토 선택도 아닌 **랜덤 추첨** — 공평 + 부담 ↓ + 차별 X

**멘토링 카드 구성 (멘토 작성)**:
1. 분야 (전공/취업/자격증 등 매칭 카테고리 재사용)
2. 주제 (자유 텍스트)
3. 모집 인원 (1~3명)
4. 모집 조건 (학년 범위 / 학과·단과대 범위)
5. 모집 기간 (3일 / 1주 / 2주)
6. 진행 형태 (1회성 / 단기 4주 / 한 학기)
7. 한 줄 소개 (멘토 본인 + 어떤 도움 가능)
8. 익명 X — 멘토 닉네임 공개 (신뢰 위해)

**멘티 지원 흐름**:
- 조건 자동 체크 (학년/학과 안 맞으면 지원 버튼 비활성)
- 한 줄 지원 메시지 (50자, 선택 — 멘토 참고용, 선택권 X)
- "지원 완료" 표시 → 결과 대기
...
- 지원자 > 모집 인원: 랜덤 추첨
- 멘토에게: 합격 멘티 닉네임 + 알림
- 멘티에게: **"지원 결과 안내"** 메일 (합격/대기 모두 차분한 톤, "당첨" 단어 X)
- 카톡 오픈채팅 안내 (멘토가 직접 링크 제공)

**🎁 멘토 동기 부여 (사용자 우려 반영)**:
- **A. 커피 안내** — 결제 X, 멘토링 안내문에 정중하게 ("음료라도 사드리면 다음 멘토에게 좋은 동기가 됩니다")
- **C. 본인 활동 통계** — "도움 준 멘티 N명" 본인만 보임
- **D. 운영자 감사 메일** — 시즌 종료 시 "OO님 덕분에 N명이 도움 받았어요" 자동 발송
- **E. 활동 인증서 PDF** ⭐ — **자체 발급** (학교 인증 X, 우리 발급)
  - "감자사이 멘토 활동 N개월 / 도움 준 멘티 N명" 작은 PDF
  - 자소서/취준 자료로 활용 가능 — 가장 강한 동기
- **수요 검증 단계**: 출시 시 티저 카드 + 알림 받기 → 멘토 N명 이상 모이면 본격 출시

**💬 질문 카드 (사용자 제안, 매칭 후 효율 ↑)**:
- 멘토가 카드 작성 시 "질문지 미리보기" 펼치기 → 기본 7개 자동 제공
  1. 관련 과목 수강 여부 + 학점
  2. 현재 수준
  3. 학교생활 (적응/친구/동아리)
  4. 고민·어려움
  5. **멘토링으로 가장 얻고 싶은 것** (핵심)
  6. 멘토에게 궁금한 것
  7. (선택) 진로 방향
- 멘토가 자유롭게 추가/삭제/수정
- 지원 결과 합격 통지 시 멘티에게 질문지 자동 전송 (인앱 + 메일)
- 멘티 답변은 **선택** (강제 X)
- 답변 분량 각 200자 제한 (부담 ↓)
- 효과: 첫 만남 효율 ↑, 멘티 진지함 검증, 멘토 만족도 ↑

**부정 행위 방지**:
- 같은 멘토링에 중복 지원 X (DB unique)
- 탈락 후 7일 안 같은 멘토 재지원 X (도배 방지)
- 멘토 본인 멘토링에 지원 X

**제외 (사용자 결정)**:
- ❌ 수료증/학교 연계 (학생 1인 운영 무리)
- ❌ 졸업생 인증 (학생증 만료 문제, 처음엔 안 받기)
- ❌ 결제/유료 (비영리)
- ❌ 선착순 / 멘토 선택 (부담 + 차별)

**DB 설계 (추후)**:
- `mentoring_cards` 테이블 (위 8개 항목 + status/created_at/closes_at + questions JSONB)
- `mentoring_applications` 테이블 (mentoring_id, mentee_id, message, status: pending/accepted/rejected)
- `mentoring_answers` 테이블 (질문지 답변, application_id별)
- `mentoring_stats` 테이블 (멘토별 도움 준 횟수 누적)
- `lottery_draw()` RPC — 모집 마감 시 랜덤 추첨
- 활동 인증서 PDF 생성 — Edge Function (Deno + PDF 라이브러리)

**정식 출시 시점 예고**:
- dashboard/커뮤니티에 작은 티저 카드 "👋 감자사이 곧 만나요"
- 알림 받기 버튼 → 출시 시 알림 메일 자동 발송
- 수요 검증 + 마케팅 효과 동시
